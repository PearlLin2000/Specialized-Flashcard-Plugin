...
- 内置向上传递的判定，即：如果 sql 检索出的块非闪卡，则检索其父块，直到检索出闪卡。
  - 比如说：你可以直接在超级块闪卡内部的段落块打一个#标签，通过#标签进行 SQL 检索，直接获取到该超级块闪卡（等价于你用#标签标记了超级块闪卡）。
  - 比如说：你可以在列表块的顶端进行块引，通过指定块的反链结果进行 SQL 检索，直接获取到完整的列表块（等价于你用[[反链标记了列表块闪卡）
- 内置闪卡检索的判定，无需在 sql 输入条件中进行闪卡条件的筛选过滤。
