...
<div class="instructions-tab">
  ...
  <div class="info-section">
    ...
    <div class="section-content">
      ...
      <div class="explanation">
        <p><strong>原理说明：</strong></p>
        <p>专项闪卡插件采用了数据缓存：定时按SQL分组向思源发送检索请求，将获取到的数据进行存储缓存。在点击闪卡复习界面的TAB时，载入缓存数据。</p>
        <p>数据缓存的优点是在点击闪卡复习界面后可以立刻复习（而不是等待几十秒），缺点如上。</p>
      </div>
      <p>你可以<u>重载思源界面</u>/<u>重启思源笔记 </u>/<u>重新点击专项闪卡插件的设置面板</u>并退出，立即触发一次缓存扫描。在等待片刻后，开启对应的闪卡复习界面。</p>
    </div>
  </div>
  ...
  <div class="info-section">
    ...
    <div class="section-content">
      <ul>
        ...
        <li>
          由于<u>数量太大导致初始载入过慢</u>干扰体验：适当调小相关参数。（后续有计划内置一个纯粹的全局到期闪卡复习Tab，限定展示数量。完成时间未定）
        </li>
      </ul>
    </div>
  </div>
